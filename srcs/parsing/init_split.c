...
static char	*type_check(char *input, char *meta)
{
	...
	while (ft_strlen(srcs) > 1 && p[1] != -1)
	{
		...
		{
			dest = ft_substr(srcs, 0, p[1]);
			dest = ft_strjoin(dest, " ");
			rest = ft_strjoin(rest, dest);
			srcs = ft_substr(srcs, p[1], p[3] - p[1]);
		}
		else if ((input[p[1] + 1] != 32) && (p[1] + 1 != p[1]))
		{
			p[2] = p[1] + 1;
			if ((srcs[p[1] + 1]) == (srcs[p[1]]))
				p[2] = p[1] + 2;
			dest = ft_substr(srcs, 0, p[2]);
			dest = ft_strjoin(dest, " ");
			rest = ft_strjoin(rest, dest);
			srcs = ft_substr(srcs, p[2], p[3] - (p[2]));
		}
		else
			break ;
	}
	rest = ft_strjoin(rest, srcs);
	input = ft_strdup(rest);
	return (input);
}
